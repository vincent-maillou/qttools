    ...
    def solve(self):
        ...
        for i in range(self.num_diags - 1):
            ...
            self.xr_diag_blocks[i + 1] = (
                self.xr_diag_blocks[i + 1] - temp_1 @ self.xr_upper_blocks[i]
            )
            if self.selected_solve:
        ...
        if self.selected_solve:
            self.xl_diag_blocks[-1] = (
                self.xr_diag_blocks[-1]
                @ self.xl_diag_blocks[-1]
                @ self.xr_diag_blocks[-1].conj().swapaxes(-2, -1)
            )
        ...
        for i in range(self.num_diags - 2, -1, -1):
            ...
            self.xr_lower_blocks[i] = -temp_3 @ self.xr_diag_blocks[i]
            self.xr_upper_blocks[i] = -temp_1 @ self.xr_diag_blocks[i + 1]
            self.xr_diag_blocks[i] = (
                self.xr_diag_blocks[i]
...
def permuted_schur(
    ...
    for i in range(1, a.num_local_blocks - 1):
        ...
        xr_buffer_lower[i] = (
            -xr_buffer_lower[i - 1] @ xr_diag_blocks[i] @ a.local_blocks[i, j]
        )
        # Update upper buffer block.
        xr_buffer_upper[i] = (
            -a.local_blocks[j, i] @ xr_diag_blocks[i] @ xr_buffer_upper[i - 1]
        )
        # Update first block.
        xr_diag_blocks[0] = (
            xr_diag_blocks[0]
            - xr_buffer_lower[i - 1] @ xr_diag_blocks[i] @ xr_buffer_upper[i - 1]
        )
        ...
        if selected_solve:
            ...
            xl_buffer_upper[i] = (
                ...
                @ xr_diag_blocks[i].conj().swapaxes(-2, -1)
                @ xr_buffer_lower[i - 1].conj().swapaxes(-2, -1)
                - a.local_blocks[j, i] @ xr_diag_blocks[i] @ xl_buffer_upper[i - 1]
            )
            xl_buffer_lower[i] = (
                xr_buffer_lower[i - 1]
                @ xl_diag_blocks[i]
            ...
            xl_diag_blocks[0] = (
                ...
                @ xr_diag_blocks[i].conj().swapaxes(-2, -1)
                @ xr_buffer_lower[i - 1].conj().swapaxes(-2, -1)
                - xr_buffer_lower[i - 1] @ xr_diag_blocks[i] @ xl_buffer_upper[i - 1]
            )
            ...
            xg_buffer_upper[i] = (
                ...
                @ xr_diag_blocks[i].conj().swapaxes(-2, -1)
                @ xr_buffer_lower[i - 1].conj().swapaxes(-2, -1)
                - a.local_blocks[j, i] @ xr_diag_blocks[i] @ xg_buffer_upper[i - 1]
            )
            xg_buffer_lower[i] = (
                xr_buffer_lower[i - 1]
                @ xg_diag_blocks[i]
            ...
            xg_diag_blocks[0] = (
                ...
                @ xr_diag_blocks[i].conj().swapaxes(-2, -1)
                @ xr_buffer_lower[i - 1].conj().swapaxes(-2, -1)
                - xr_buffer_lower[i - 1] @ xr_diag_blocks[i] @ xg_buffer_upper[i - 1]
            )
